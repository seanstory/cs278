...
public class SMSReceiver extends BroadcastReceiver {
	...
	private static final CmdMapping cmdMap = new DefaultCommandMapping();
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (cmds.size() == 0) {
			...
			for (String cmd : cmds) {
				String prefix = cmd.split("\\s+")[0];//split by whitespace
				Command curCommand = null; //declare varible and initialize in scope.
				int start = cmd.indexOf("\"");
				int end = cmd.indexOf("\"", start + 1);
				String msg = cmd.substring(start + 1, end);
				Object[] args = {context,msg,Toast.LENGTH_LONG};
				
				try {//try to instantiate a command based on what we've got.
					curCommand = (Command) cmdMap.getCommand(prefix).getConstructors()[0].newInstance(args);
